...
const addProduct = async (req, res) => {
  try {
    ...
    //--------------------------------------------------Artwork Details------------------------------------//
      ...
      medium: value.medium?.value || value.medium?.label,
      materials: value.materials?.map((m) => m.value || m.label) || [],
      dimensions: {
        width: parseFloat(value.width),
        height: parseFloat(value.height),
        depth: parseFloat(value.depth),
      },
      weight: value.weight ? parseFloat(value.weight) : undefined,
      printResolution:
        (value.medium?.value?.toLowerCase() === 'print' || value.medium?.value?.toLowerCase() === 'poster')
          ? value.printResolution
          : undefined,
      year: value.year?.value,
      editionType: value.editionType?.value,
      framing: value.framing?.value,
      quantity: parseInt(value.quantity),
      hsnCode: value.hsnCode || undefined,
      surfaceType: value.surfaceType?.value || value.surfaceType?.label || undefined,
      culturalRegion: value.culturalRegion?.value || value.culturalRegion?.label || undefined,
      biologicalMaterial: value.biologicalMaterial || undefined,
      functionalUse: value.functionalUse?.value,
      materialSource: value.materialSource || undefined,
      craftTechnique: value.craftTechnique || undefined,
      toolUsage: value.toolUsage?.map((t) => t.value || t.label) || undefined,
      handmade: value.handmade?.value,
      isSigned: value.isSigned,
      isResinCovered: value.isResinCovered || false,
      condition: value.condition?.value,
      provenance:
        value.condition?.value?.toLowerCase() === 'resale'
          ? value.provenance
          : undefined,
      ...
      handlingTime: value.handlingTime,
      ...
      exportRestriction: value.exportRestriction || false,
  ...
  commercialUse: value.commercialUse || 'no',
  royaltyTerms: value.royaltyTerms || '',
  ethicalSourcing: !!value.ethicalSourcing,
      installmentDuration: value.allowInstallments
        ? value.installmentDuration || []
        : [],
    };
...
//             </span>
//           </div>
//           ${product.productType === "Limited Edition"
//           ? `
//           <div style="margin-bottom: 12px; display: flex;">
