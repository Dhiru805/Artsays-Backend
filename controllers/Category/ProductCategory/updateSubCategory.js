...
const { subCategoryName, categoryId, mainCategoryId, commissionTerm } = req.body;
...
if (commissionTerm === undefined || commissionTerm === null) {
  return res.status(400).json({
    hasError: true,
    message: "Commission term is required for update.",
  });
}
        ...
        const updatedSubCategory = await SubCategory.findByIdAndUpdate(
          id,
          { $set: { subCategoryName, categoryId, mainCategoryId, commissionTerm  } },
          { new: true }
        )
          .populate({
            path: "categoryId",
        ...
        return res.status(200).json({
          ...
          data: {
            ...
            categoryName: updatedSubCategory.categoryId?.categoryName || null,
              commissionTerm: updatedSubCategory.commissionTerm || null,
          },
        });
      } catch (error) {
