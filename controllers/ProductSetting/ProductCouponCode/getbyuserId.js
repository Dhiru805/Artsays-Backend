...
const getProductCouponCodesbyuserid = async (req, res) => {
  try {
    const { userId } = req.params; 
    const query = userId ? { userId } : {};

    const productCouponCodes = await ProductCouponCode.find(query)
      .populate({
        path: "userId",
...
module.exports = getProductCouponCodesbyuserid;