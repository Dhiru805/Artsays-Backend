...
const GetSuggestedUsersProfile = async (req, res) => {
  try {
    ...
    if (categories.length > 0) {
      ...
      for (const u of candidates) {
        ...
        if (suggestedUsers.length >= 10) break;
      }
    }

    // 10️⃣ Fill remaining spots with random users
    if (suggestedUsers.length < 10) {
      ...
      const fillers = await User.aggregate([
        ...
        { $sample: { size: 10 - suggestedUsers.length } },
...
module.exports = GetSuggestedUsersProfile;
