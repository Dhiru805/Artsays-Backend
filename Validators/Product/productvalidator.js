...
const productValidator = Joi.object({
  ...
  targetedAudience: Joi.string().required().trim().messages({
    'string.empty': 'Targeted audience is required',
    'any.required': 'Targeted audience is required'
  }),
  inspirationSource: Joi.string().required().trim().messages({
  ...
  width: Joi.string().required().custom((value, helpers) => {
    const num = parseFloat(value);
    if (isNaN(num) || num <= 0) return helpers.error('number.base');
    return num;
  }).messages({
    'any.required': 'Width is required',
    'number.base': 'Width must be a positive number'
  }),
  height: Joi.string().required().custom((value, helpers) => {
    const num = parseFloat(value);
    if (isNaN(num) || num <= 0) return helpers.error('number.base');
    return num;
  }).messages({
  ...
  depth: Joi.string().required().custom((value, helpers) => {
    const num = parseFloat(value);
    if (isNaN(num) || num < 0) return helpers.error('number.base');
    return num;
  }).messages({
  ...
  printResolution: Joi.string()
    ...
    .allow('')
    .when('medium', {
      is: Joi.string().valid('print', 'poster').insensitive(),
      then: Joi.string().required().messages({
        'string.empty': 'Print resolution is required for print or poster mediums',
        'any.required': 'Print resolution is required for print or poster mediums',
      }),
    }),
  year: Joi.number()
    .integer()
  ...
  editionType: Joi.string().required().messages({
    'any.required': 'Edition type is required',
  }),
  framing: Joi.string()
    .valid('framed', 'unframed', 'rolled')
    .required()
    .messages({
  ...
  hsnCode: Joi.string().trim().allow('').messages({
    'string.empty': 'HSN Code cannot be empty if provided',
  }),
  surfaceType: Joi.string().trim().allow('').messages({
    'string.empty': 'Surface type cannot be empty if provided',
  }),
  culturalRegion: Joi.string().trim().allow('').messages({
    'string.empty': 'Cultural region cannot be empty if provided',
  }),
  biologicalMaterial: Joi.string().trim().allow('').messages({
    'string.empty': 'Biological material cannot be empty if provided',
  }),
  functionalUse: Joi.string()
    .valid('yes', 'no')
    .required()
    .messages({
      'any.only': 'Invalid functional use value',
      'any.required': 'Functional use is required',
    }),
  materialSource: Joi.string().trim().allow('').messages({
    'string.empty': 'Material source cannot be empty if provided',
  }),
  craftTechnique: Joi.string().trim().allow('').messages({
    'string.empty': 'Craft technique cannot be empty if provided',
  }),
  toolUsage: Joi.array().items(Joi.string()).messages({
    'array.base': 'Tool usage must be an array of strings',
  }),
  handmade: Joi.string()
    .valid('yes', 'no')
    .required()
    .messages({
      'any.only': 'Invalid handmade value',
      'any.required': 'Handmade status is required',
    }),
  isSigned: Joi.boolean().required().messages({
    'any.required': 'Signature status is required',
  }),
  isResinCovered: Joi.boolean().default(false).messages({
    'boolean.base': 'Resin coverage must be a boolean value',
  }),
  condition: Joi.string()
    .valid('new', 'resale', 'pre_owned')
    .required()
    .messages({
  ...
  provenance: Joi.string()
    ...
    .when('condition', {
      is: 'resale',
      then: Joi.string().required().messages({
        'string.empty': 'Provenance is required for resale condition',
        'any.required': 'Provenance is required for resale condition',
      }),
    }),
  ...
  installmentDuration: Joi.string()
    .when('allowInstallments', {
      is: true,
  ...
  gstPercentage: Joi.number().min(0).max(100).default(0).messages({
  ...
  handlingTime: Joi.string().required().messages({
    'string.empty': 'Handling time is required',
    'any.required': 'Handling time is required'
  }),
  ...
  returnPolicy: Joi.string()
    ...
    .required()
    .messages({
      'any.only': 'Return policy must be either returnable or non-returnable',
  ...
  // Legal & Compliance validations
   ...
   commercialUse: Joi.string().valid('yes', 'no').default('no').messages({
     'any.only': 'Commercial use must be either "yes" or "no"'
   }),
...
conservationStatus: Joi.string()
  .valid('restored', 'original')
  .when('mainCategory', {
    is: 'Antiques & Vintage',
    then: Joi.required().messages({
      'any.required': 'Conservation Status is required for antiques',
      'any.only': 'Invalid conservation status value'
    }),
    otherwise: Joi.optional()
  }),
...
isHandmade: Joi.boolean()
  .default(false)
  .when('mainCategory', {
    is: 'Antiques & Vintage',
    then: Joi.required().messages({
      'any.required': 'Handmade status is required for antiques'
    }),
    otherwise: Joi.optional()
  }),
...
maintenanceRequired: Joi.string()
  .valid('yes', 'no')
  .when('mainCategory', {
    is: 'Antiques & Vintage',
    then: Joi.required().messages({
      'any.required': 'Maintenance requirement is required for antiques',
      'any.only': 'Invalid maintenance requirement value'
    }),
    otherwise: Joi.optional()
  }),
